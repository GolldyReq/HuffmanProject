...
1er argument : compress OU decompress   /!\OBLIGATOIRE/!\
               (l'argument both compress et decompresse le fichier <- uniquement pour faire des tests dev)
...
autres argument : -w (write) nom_fichier-> ecrit la decompression dans un fichier txt au lieu de l'afficher sur la console (optionnel)
                 ...
                 -v pour le mode verbose avec l'affichage de la table de Huffman(optionnel)
                 -V pour le mode verbose avec l'affichage de l'arbre de Huffman ne marche que pour compress(optionnel)
                 -e pour afficher les entetes du fichiers (nb caractere total, nb caractere distinct...) (ne marche que pour decompress) (oprionnel)
**/
...
int main(int nbarg,char **argv)
{
    ...
    {
        compress_file(argv,nbarg);
        decompress_file(argv,nbarg);
        exit(0);
    }
